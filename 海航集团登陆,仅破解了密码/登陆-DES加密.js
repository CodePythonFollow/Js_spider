...
(function () {
    ...
    u.enc.Base64 = {
        stringify: function (d) {
                ...
                d = [];
                for (var q = 0; q < l; q += 3)
                    for (var w = (n[q >>> 2] >>> 24 - 8 * (q % 4) & 255) << 16 | (n[q + 1 >>> 2] >>> 24 - 8 * ((q + 1) % 4) & 255) << 8 | n[q + 2 >>> 2] >>> 24 - 8 * ((q + 2) % 4) & 255, v = 0; 4 > v && q + .75 * v < l; v++)
...
function text(key, iv, pd) {
    key = CryptoJS.enc.Utf8.parse(key);
    iv = CryptoJS.enc.Utf8.parse(iv);
    // DES 加密
    var encrypted = CryptoJS.DES.encrypt(pd, key, {
        iv: iv,
